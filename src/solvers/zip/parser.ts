...
export async function parseZipBoard(context: Page | Frame): Promise<ZipBoard> {
  ...
  const boardSelector = '.trail-grid, .grid-game-board, [data-trail-grid]';
  ...
  const cellData = await context.evaluate(() => {
    // Find all trail-cell elements (exclude border elements)
 ...
 * Walls are represented as .trail-cell-wall elements inside each .trail-cell,
 * with direction classes like --down, --right, --left, --up.
 */
async function extractWalls(context: Page | Frame, cols: number): Promise<ZipWalls> {
  const wallData = await context.evaluate((gridCols: number) => {
    ...
    for (let i = 0; i < cells.length; i++) {
      const cell = cells[i];
      const idx = parseInt(cell.getAttribute('data-cell-idx') || String(i), 10);
      const wallEls = cell.querySelectorAll('.trail-cell-wall');
      const directions: string[] = [];

      for (const w of wallEls) {
        const cls = w.className;
        // Check for pure directional walls (not corner decorations)
